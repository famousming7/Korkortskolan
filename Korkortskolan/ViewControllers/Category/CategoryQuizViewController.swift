...
import Parse
class CategoryQuizViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    @IBOutlet weak var lbl_resultTime: UILabel!
...
//        completedPractice()
    ...
    func getDBQuiz(){
        ...
        let query = PFQuery(className: "Category_Quiz")
        ...
        query.findObjectsInBackground(block: { (quizs : [PFObject]?, error: Error!) -> Void in
            if error == nil {
                ...
                if(self.m_arrQuiz.count == 0){
                    Helper.showAlert(target: self, title:"Warning!" , message: "Please wait. Category Quizes were not uploaded.", completion:{
                        self.navigationController?.popViewController(animated: true)
                    })
                }
...
//        lbl_resultTime.text = "TIME: " + twoCharacters(val: min) + ":" + twoCharacters(val: sec)
        ...
        resultView.category_id = category_id
    ...
    func saveQuizResult(){
        ...
        UserDefaults.standard.setValue(archivedData, forKey: "category_quiz_res_" + category_id)
    ...
    func saveHistory(){
        ...
        let history_title = category_title + " Final"
